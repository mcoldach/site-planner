-- Classifies a raw zoning string into base / overlay / combined / unclassified
-- by looking each token up in zone_registry for the given jurisdiction.
-- Tokenizes on whitespace AND slash (so 'BP/CR CU HS SS' -> BP, CR, CU, HS, SS).
-- Single canonical classification path; get_parcel_context and bulk jobs both call this.
